...
const Rooms = () => {
  ...
  const getAllData = async () => {
    try {
      ...
      const roomsWithCount = data.rooms.map((guest, index) => ({
        ...guest,
        count: index + 1,
      }));
      ...
      Cell: ({ row }) => (
        <div className="w-full flex justify-center">
          <button
            ...
            className="px-4 py-2 rounded-lg text-center trnasition-effect text-white bg-zinc-900 border-2 border-blue-900 hover:bg-blue-900 hover:scale-105"
          >
            <MdDelete />
          </button>
        </div>
    ...
    <>
      <div className="flex flex-col justify-center items-center ">
        <div className="w-full  bg-blue-700 text-center ">
          <h1 className="text-white text-2xl font-bold p-8 ">Rooms</h1>
        </div>
        <div></div>
        <div className="bg-slate-100 p-12 rounded-xl mt-10 w-5/6 ">
          <button
            onClick={toggleCreateModal}
            className="p-2 bg-blue-600 text-white rounded-md float-end translate-effect hover:scale-105 hover:bg-blue-500"
          >
            Create New
          </button>
